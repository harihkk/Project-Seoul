...
| Peek | Lightweight preview of a link, promotable to a real tab | Glance without committing | No tab clutter | Ambiguous when a Peek becomes permanent | Source-connected native Preview host with explicit tab/split promotion; compile/runtime evidence pending | A preview never silently becomes a retained tab |
